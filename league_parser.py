...
def attach_badges(players, rows, base_row_index):
    if not players:
        return players
...
def extract_header_stats_from_row(row):
    ...
    for t in normalized[kda_index + 1 :]:
        ...
        for num in nums:
            ...
            if len(num) == 1:
                objective_digits.append(int(num))
            elif len(num) == 2:
                if not objective_digits:
                    objective_digits.append(int(num))
                else:
                    objective_digits.extend(int(ch) for ch in num)
            else:
                objective_digits.extend(int(ch) for ch in num)
...
def parse_image(image_path, match_id, game_number):
    ...
    print("\n=== PARSED MATCH SUMMARY START ===")
    print(json.dumps(payload, indent=2, ensure_ascii=False, sort_keys=True))
    print("=== PARSED MATCH SUMMARY END ===")
